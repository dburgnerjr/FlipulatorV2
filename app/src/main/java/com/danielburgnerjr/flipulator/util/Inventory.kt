...
class Inventory internal constructor() {
    internal var mSkuMap: MutableMap<String, SkuDetails> = HashMap<String, SkuDetails>()
    internal var mPurchaseMap: MutableMap<String, Purchase> = HashMap<String, Purchase>()
    ...
    internal val allPurchases: List<Purchase>
        get() = ArrayList<Purchase>(mPurchaseMap.values)
    ...
    internal fun getAllOwnedSkus(itemType: String): List<String> {
        ...
        for (p in mPurchaseMap.values) {
            if (p.itemType.equals(itemType)) result.add(p.sku)
        }
        return result
    }
